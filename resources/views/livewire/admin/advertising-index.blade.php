<div class="vs jj ttm vl ou uf na">
    ...
    <div class="je jd jc ii">
        ...
        <!-- Left: Title -->
        <div class="ri _y">
            <h1 class="gu teu text-slate-800 font-bold">Advertising</h1>
        </div>
        ...
        <div class="sn am jo az jp ft">
            ...
            <button class="btn ho xi ye" wire:click="showCreateModal">
                ...
                <span class="hidden trm nq">Create Advertising</span>
            </button>
        </div>
    ...
    <div class="bg-white bd rounded-sm border border-slate-200 rc">
        <header class="vc vu">
            <h2 class="gh text-slate-800">Advertisings <span class="gq gp"></span></h2>
        </header>
                ...
                <table class="ux ou">
                    ...
                    <tbody class="text-sm le lr">
                        ...
                        <tr>
                            ...
                            <td class="vi wy w_ vo lm">
                                <div class="gp ">
                                    @if ($advertising->small)
                                    <img src="{{ asset('storage/'.$advertising->small) }}" class="object-scale-down h-48 w-96" alt="{{ $advertising->title }}">
                                    @endif
                                </div>
                            </td>
                            <td class="vi wy w_ vo lm">
                            ...
                            <td class="vi wy w_ vo lm">
                                @if ($advertising->status === 'inactive')
                                <div class="inline-flex gp hf yl rounded-full gn vp vd">{{ $advertising->status }}</div>
                                @endif

                                @if ($advertising->status === 'active')
                                <div class="inline-flex gp hc ys rounded-full gn vp vd">{{ $advertising->status }}</div>
                                @endif
                            </td>
                            <td class="vi wy w_ vo lm">
                                <div>{{ $advertising->created_at->format('d-m-Y') }}</div>
                            </td>

                            <td class="vi wy w_ vo lm of">
                                <div class="fm">
                                    <button class="gq xv rounded-full" wire:click="showEditModal({{ $advertising->id }})">
                                    ...
                                    <button class="yl xy rounded-full" wire:click="deleteModal({{ $advertising->id }})">
    ...
    <x-dialog-modal wire:model="showAdvertisingModal" class="">

        @if ($advertisingId)
        <x-slot name="title" class="border-b">Update Advertising</x-slot>
        @else
        <x-slot name="title" class="border-b bg-slate-200">
            <span class="font-semibold">Create Advertising</span>
        </x-slot>
        @endif
        ...
        <x-slot name="footer">
            <div class="border-slate-200">
                <div class="flex flex-wrap justify-end fc">
                    <x-button wire:click="closeAdvertisingModal" class="border-slate-200 hover:text-white hover--border-slate-300 g_">Cancel</x-button>
                    @if ($advertisingId)
                    <x-button wire:click="updateAdvertising" class=" ho xi ye">Update</x-button>
                    @else
                    <x-button wire:click="createAdvertising" class=" ho xi ye2">Create</x-button>
                    @endif
                </div>
            </div>
