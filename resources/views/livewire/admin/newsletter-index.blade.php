...
<x-loading-indicator />
    ...
    <div class="je jd jc ii">
        ...
        <!-- Left: Title -->
        <div class="ri _y">
            <h1 class="gu teu text-slate-800 font-bold">Newsletters</h1>
        </div>
    ...
    <div class="bg-white bd rounded-sm border border-slate-200 rc">
        <header class="vc vu">
            <h2 class="gh text-slate-800">Newsletter List<span class="gq gp"></span></h2>
        </header>
        <div x-data="">
                            ...
                            <th class="vi wy w_ vo lm of">
                                <div class="flex items-center">
                                    <label class="inline-flex">
                                        ...
                                        <input type="hidden" wire:model="firstId"  />
                                    </label>
                                </div>
                            </th>
                        ...
                        <tr>
                            ...
                            <td class="vi wy w_ vo lm">
                                @if ($s->status === 'inactive')
                                    <div class="inline-flex gp hf yl rounded-full gn vp vd">{{ $s->status }}</div>
                                @endif 

                                @if ($s->status === 'active')
                                    <div class="inline-flex gp hc ys rounded-full gn vp vd">{{ $s->status }}</div>
                                @endif 
                            </td>
                            ...
                            <td class="vi wy w_ vo lm of">
                                <div class="fm">
                                    <button class="gq xv rounded-full" wire:click="showEditModal({{ $s->id }})">
                                    ...
                                    <button class="yl xy rounded-full" wire:click="deleteModal({{ $s->id }})">
    ...
    <x-dialog-modal wire:model="showNewsletterModal" class="">

        @if ($newsletterId)
        <x-slot name="title" class="border-b">Update Subscribe</x-slot>
        @else
        <x-slot name="title" class="border-b bg-slate-200">
            <span class="font-semibold">Create Subscribe</span>
        </x-slot>
        @endif
                                ...
                                <div class="">
                                    <div class="flex flex-col space-y-3">
                                        ...
                                        <div class="col-start-1 sm:col-span-3">
                                            ...
                                            <input wire:model="email" type="text" autocomplete="given-name" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md" />
                                        </div>
                                        
                                        <div class="col-span-6 sm:col-span-3">
                                            <label for="first-name" class="block text-sm font-medium text-gray-700">Status</label>
                                            <select wire:model="subscribeStatus" class="h-full rounded-r border-t border-r border-b block appearance-none w-full bg-white border-gray-300 text-gray-700 py-2 px-4 pr-8 leading-tight focus:placeholder-gray-600 focus:text-gray-700 focus:outline-none">
        ...
        <x-slot name="footer">
            <div class="border-slate-200">
                <div class="flex flex-wrap justify-end fc">
                    <x-button wire:click="closeFaqModal" class="border-slate-200 hover:text-white hover--border-slate-300 g_">Cancel</x-button>
                    @if ($newsletterId)
                    <x-button wire:click="updateFaq" class=" ho xi ye">Update</x-button>
                    @else
                    <x-button wire:click="createFaq" class=" ho xi ye2">Create</x-button>
                    @endif
                </div>
            </div>
