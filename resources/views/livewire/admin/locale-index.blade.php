...
<x-loading-indicator />
    ...
    <div class="je jd jc ii">
        ...
        <!-- Left: Title -->
        <div class="ri _y">
            <h1 class="gu teu text-slate-800 font-bold">Language</h1>
        </div>
        ...
        <div class="sn am jo az jp ft">
            ...
            <button class="btn ho xi ye" wire:click="showCreateModal">
                ...
                <span class="hidden trm nq">Create Language</span>
            </button>
        </div>
    ...
    <div class="bg-white bd rounded-sm border border-slate-200 rc">
        <header class="vc vu">
            <h2 class="gh text-slate-800">Locales <span class="gq gp"></span></h2>
        </header>
                ...
                <table class="ux ou">
                    ...
                    <thead class="go gh gv text-slate-500 hp co cs border-slate-200">
                        <tr>
                            ...
                            <th class="vi wy w_ vo lm">
                                <div class="gh gt">Code</div>
                            </th>
                            <th class="vi wy w_ vo lm">
                                <div class="gh gt">Name</div>
                            </th>
                            <th class="vi wy w_ vo lm">
                                <div class="gh gt">Rtl</div>
                            </th>
                            <th class="vi wy w_ vo lm">
                        ...
                        <tr>
                            ...
                            <td class="vi wy w_ vo lm">
                                <div class="gp text-slate-800">
                                    {{ ($languages->currentpage()-1) * $languages->perpage() + $loop->index + 1 }}
                                </div>
                            </td>
                            <td class="vi wy w_ vo lm">
                                <div class="gp text-slate-800">{{ $language->code }}</div>
                            </td>
                            <td class="vi wy w_ vo lm">
                                <div class="gp text-slate-800">{{ $language->name }}</div>
                            </td>
                            <td class="vi wy w_ vo lm">
                                <div class="gp ">{{ $language->rtl == true ? 'true' : 'false' }}</div>
                            </td>
                            <td class="vi wy w_ vo lm">
                                @if ($language->status === 'inactive')
                                    <div class="inline-flex gp hf yl rounded-full gn vp vd">{{ $language->status }}</div>
                                @endif 

                                @if ($language->status === 'active')
                                    <div class="inline-flex gp hc ys rounded-full gn vp vd">{{ $language->status }}</div>
                                @endif 
                            </td>
                            
                            <td class="vi wy w_ vo lm of">
                                <div class="fm">
                                    <button class="gq xv rounded-full" wire:click="showEditModal({{ $language->id }})">
                                    ...
                                    <button class="yl xy rounded-full" wire:click="deleteModal({{ $language->id }})">
    ...
    <x-dialog-modal wire:model="showLanguageModal" class="">

        @if ($languageId)
        <x-slot name="title" class="border-b">Update Language</x-slot>
        @else
        <x-slot name="title" class="border-b bg-slate-200">
            <span class="font-semibold">Create Language</span>
        </x-slot>
        @endif
                                    ...
                                    <div class="flex flex-row justify-between space-x-2">
                                        ...
                                        <div class="flex space-x-22 justify-between space-x-2">
                                            <div class="col-span-6 sm:col-span-3 w-full">
                                                <label for="religionId" class="block text-sm font-medium text-gray-700 pb-1">RTL</label>
                                                <select wire:model="rtl" class="h-10 border block appearance-none w-full bg-white border-gray-600 text-gray-700 py-2 px-4 pr-0 leading-tight focus:placeholder-gray-600 focus:text-gray-700 focus:outline-none">
                                                    <option value="">Select Option</option>
                                                    @foreach($rtlOption as $i => $v)
                                            ...
                                            <div class="col-span-6 sm:col-span-3 w-full">
                                                ...
                                                <select wire:model="langStatus" class="h-10 border block appearance-none w-full bg-white border-gray-600 text-gray-700 py-2 px-4 pr-0 leading-tight focus:placeholder-gray-600 focus:text-gray-700 focus:outline-none">
                                                    <option value="" >Select Option</option>
        ...
        <x-slot name="footer">
            <div class="border-slate-200">
                <div class="flex flex-wrap justify-end fc">
                    <x-button wire:click="closeLanguageModal" class="border-slate-200 hover:text-white hover--border-slate-300 g_">Cancel</x-button>
                    @if ($languageId)
                    <x-button wire:click="updateLanguage" class=" ho xi ye">Update</x-button>
                    @else
                    <x-button wire:click="createLanguage" class=" ho xi ye2">Create</x-button>
                    @endif
                </div>
            </div>
