...
<div class="relative inline-flex" x-data="{ open: false }">
    <button
        ...
        :aria-expanded="open"                        
    >
        <img class="w-8 h-8 rounded-full" src="{{ Avatar::create(ucwords(Auth::user()->name))->toBase64() }}" width="32" height="32" alt="{{ Auth::user()->name }}" />
        <div class="flex items-center truncate">
            <span class="truncate ml-2 text-sm font-medium group-hover:text-slate-800">{{ ucwords(Auth::user()->name) }}</span>
    ...
    >
        <div class="pt-0.5 pb-2 px-3 mb-1 border-b border-slate-200">
            <div class="font-medium text-slate-800">{{ ucwords(Auth::user()->name) }}</div>
