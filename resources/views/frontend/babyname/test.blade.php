...
<main class="container mx-auto px-4 py-8"> {{-- Added container and padding for better layout --}}
    <div x-data="fullNameGenerator()">
        <h1 class="text-3xl font-bold mb-6 text-center">Full Name Generator</h1>
        ...
        <div class="bg-white p-6 rounded-lg shadow-md mb-8">
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-4">
                ...
                <div>
                    ...
                    <select x-model.number="filters.num_names" id="num_names" class="shadow border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
            ...
            {{-- Origins Filter (Single Select as per your provided code) --}}
            <div class="mb-4">
                        ...
                        <div class="flex flex-wrap gap-x-4 gap-y-2">
                            <template x-for="(namePart, nameIndex) in combination" :key="namePart.slug + '-' + nameIndex">
                                <div
                                    ...
                                    x-ref="namePartRef"
                                >
                                    <span class="font-medium" x-text="namePart.name"></span>
                                    ...
                                    >
                                        <p class="font-bold mb-1" x-text="namePart.name"></p>
                                        <p>Meaning: <span x-text="namePart.meaning"></span></p>
                                        <p>Slug: <span x-text="namePart.slug"></span></p>
                                    </div>
                                </div>
                            </template>
                        </div>
...
<script>
    function fullNameGenerator() {
        return {
            filters: {
                num_names: 2,
                origins: '', // Changed to single string for single select
                gender: '',
                limit: 10,
            },
            // availableOrigins: [], // No longer needed as it's passed from Blade to the select
            suggestions: [],
            loading: false,
            error: null,

            // No need for fetchOrigins() here as data is passed directly from Laravel

            // Initial setup for filters
            init() {
                // If you want to trigger initial generation on page load:
                // this.generateNames();
            },
            ...
            async generateNames() {
                ...
                try {
                    ...
                    const response = await fetch('{{ route("full_name_generator.generate") }}', {
                        ...
                        body: JSON.stringify(payload) // Send filters as JSON body
                    });
                    ...
                    this.error = 'Network error or unable to connect to server. Please check your internet connection.';
                } finally {
                    this.loading = false;
                }
            }
