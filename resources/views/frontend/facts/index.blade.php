...
<main class="flex bg-white min-h-screen2 h-full pt-20 md:pt-[80px] max-w-[85rem] px-4 py-10 sm:px-6 lg:px-8 lg:py-14 mx-auto">
    <div class="flex mx-auto max-w-lg md:max-w-2xl w-full grid2 grid-cols-122  gap-6 mt-6 h-full">
        <div class="relative col-span-2 w-full h-full2" x-data="carousel()">

            <div
                class="flex w-full h-full2  overflow-x-scroll2 overflow-hidden scroll-smooth gap-0 snap-x snap-mandatory"
                x-ref="slider"
                @keydown.right="next"
                @keydown.left="prev">
                <!-- product -->
                ...
                <div class="w-full h-full2 span-start flex-shrink-0 @if($i++ == 1){{ 'col-span-2' }}@else{{ '' }}@endif">
                    <div
                        class="relative mb-4 rounded w-full h-0 pt-1/2 pb-1/2 border group flex flex-col overflow-hidden justify-center shadow-md items-center"
                        ...
                        <div class="flex flex-col justify-center items-center px-10 md:px-20 py-6 lg:py-4 md:py-6">
                            <svg xmlns="http://www.w3.org/2000/svg" version="1.1" viewBox="0 0 2048 2048" stroke-width="3" width="100" height="100">
            ...
            <div class="flex z-20">
                <button @click="prev" class="flex items-center justify-center text-3xl font-medium absolute top-1/2 transform -translate-y-1/2 left-0 bg-white p-1">
                ...
                <button @click="next" class="text-3xl font-medium absolute top-1/2 transform -translate-y-1/2 right-0 bg-white p-1">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="3" stroke="currentColor" class="size-6 md:size-11">
...
{{--
    ...
    <div 
        class="flex overflow-x-scroll2 overflow-hidden scroll-smooth  gap-0 snap-x snap-mandatory" 
        x-ref="slider"
        @keydown.right="next"
        @keydown.left="prev"
    >
        ...
        @endphp
        @foreach($facts as $f)
        <div class="w-full span-start flex-shrink-0 @if($i++ == 1){{ 'col-span-2' }}@else{{ '' }}@endif">
<img src="/fact/{{ $f }}" alt="" class="object-cover w-full" />
</div>
@endforeach
</div>
