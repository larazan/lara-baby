...
<aside @click="showContent = !showContent" class="fixed flex w-[30px] right-0 left-2 top-1/2 transform -translate-y-1/2 z-20 cursor-pointer">
...
<template x-if="true">
<aside x-show="showContent" @click.away="showContent = false" x-transition:enter="motion-safe:ease-out duration-300" x-transition:enter-start="opacity-0 scale-90" x-transition:enter-end="opacity-100 scale-100" @keydown.escape="showContent = false" x-cloak class="fixed flex w-[200px] max-h-[200px] overflow-auto right-0 left-11  top-1/2 transform -translate-y-1/2 z-30">
    <div class="relative w-full py-1 shadow-2xl rounded-lg border bg-[#f0f0f0] ">
        <div class=" flex flex-col items-center2 w-full h-full px-2 py-1 overflow-y-auto">
            <div class="flex items-start text-gray-500 text-[11px] font-sans uppercase tracking-wide">
                <span>contents</span>
            </div>
            <div class="flex flex-col py-2 px-2 space-y-1 text-sm text-gray-500 ">
               {!! $contents['index'] !!}
